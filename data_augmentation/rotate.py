...
class yoloRotatebbox:
    ...
    def rotate_image(self):
        ...
        image_center = (width / 2,
                        height / 2)  # getRotationMatrix2D needs coo#jlfs oışsdttdağ- ğ komple değiştrme rdinates in reverse order (width, height) compared to shape
...
if __name__ == "__main__":
    ...
    rotated_image_filename = '/train/images/' + os.path.basename(image_path).split('.')[0] + '_' + str(angle) + '.jpg'
    cv2.imwrite(rotated_image_filename, image)
    file_name = 'a/train/labels/' + os.path.basename(image_path).split('.')[0]+'_' + str(angle) + '.txt'
